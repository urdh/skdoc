...
if [[ "$TRAVIS_TAG" ]]; then
  ...
  [[ "$CONTRIBUTION" && "$NAME" && "$EMAIL" && "$DIRECTORY" ]] || exit 2;
  ...
  export CONTRIBUTION NAME EMAIL SUMMARY DIRECTORY NOTES LICENSE FREEVERSION FILE
  # 4. Upload package
  ctanupload -y --version=$TRAVIS_TAG --notes="Package contains TDS zip file" --DoNotAnnounce=1
fi
